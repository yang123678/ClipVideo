...
namespace BXC {
    ControlExecutor::ControlExecutor(Control* control) :
        mControl(control),
        mPullStream(nullptr),
        mPushStream(nullptr),
        mAnalyzer(nullptr),
    ...
    bool ControlExecutor::start() {

        mControl->executorStartTimestamp = getCurTimestamp();

        this->mPullStream = new AvPullStream(mControl);
        if (this->mPullStream->connect()) {
     
            this->mPushStream = new AvPushStream(mControl);
            if (this->mPushStream->connect()) {
                // success
            }
            else {
                LOGE("pull stream connect success, push stream connect error");
                return false;
            }
        }
        else {
            LOGE("pull stream connect error");
            return false;
        }

        this->mAnalyzer = new Analyzer(mControl);
